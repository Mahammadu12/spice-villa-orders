  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="fixed inset-0 -z-10">
        <img src={menuCollageBg} alt="" className="w-full h-full object-cover opacity-[0.07]" aria-hidden="true" />
      ...
      <div className="max-w-4xl mx-auto px-6 pt-32 pb-28">
        ...
        <motion.div className="text-center mb-16" initial="hidden" animate="visible" variants={fadeUp}>
          <p className="text-primary/60 text-[10px] font-medium tracking-[0.5em] uppercase mb-4">
          ...
          <p className="text-white/40 max-w-sm mx-auto text-sm leading-relaxed italic font-light">
            {t(
                ...
                <motion.section key={category.title} variants={fadeUp}>
                  ...
                  {catImage ? (
                    <div className="relative rounded-2xl overflow-hidden mb-10 ring-1 ring-white/[0.06]">
                      <div className="relative h-48 md:h-60">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-7 md:p-9">
                          <p className="text-primary/60 text-[10px] tracking-[0.4em] uppercase mb-2 font-medium">
                            {t("Kategori", "Category")}
                          </p>
                          <h2 className="font-serif text-3xl md:text-4xl text-white drop-shadow-lg tracking-tight">{displayTitle}</h2>
                          {displayDesc && (
                            <p className="text-white/50 text-sm mt-2 max-w-lg italic font-light">{displayDesc}</p>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mb-10 text-center">
                      <p className="text-primary/50 text-[10px] tracking-[0.4em] uppercase mb-2 font-medium">
                        {t("Kategori", "Category")}
                      </p>
                      <h2 className="font-serif text-3xl md:text-4xl text-white tracking-tight">{displayTitle}</h2>
                      {displayDesc && <p className="text-white/40 text-sm mt-2 italic font-light">{displayDesc}</p>}
                      ...
                      return (
                        <div
                          ...
                          className={`group py-4 ${
                            i !== category.items.length - 1 ? "border-b border-white/[0.04]" : ""
                          }`}
                        >
                          <div className="flex items-baseline gap-2">
                            {item.num && (
                              <span className="text-primary/30 text-[11px] font-mono w-6 flex-shrink-0">{item.num}.</span>
                            )}
                            <h3 className="text-white/90 font-serif text-base md:text-lg tracking-wide group-hover:text-primary transition-colors duration-300">
                              {itemName}
                            </h3>
                            <span className="flex-1 border-b border-dotted border-white/[0.08] mx-3 mb-1 min-w-[2rem]" />
                            <span className="text-primary/80 font-serif text-base md:text-lg whitespace-nowrap tracking-wide">
                              {item.price}
                            </span>
                          </div>
                          {itemDesc && (
                            <p className={`text-white/30 text-xs mt-1.5 leading-relaxed italic font-light ${item.num ? "pl-8" : ""}`}>
                              {itemDesc}
                            </p>
                          )}
                        </div>
        ...
        >
          <Ornament className="mb-6" />
          <p className="text-white/20 text-[10px] mt-4 tracking-[0.3em] uppercase font-medium">
