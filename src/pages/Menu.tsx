        ...
        <div className="flex flex-wrap justify-center gap-2 mb-12">
          ...
          {dinnerMenu.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-medium tracking-wide uppercase transition-colors ${
                activeCategory === cat.title ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"
              }`}
            >
              {cat.title}
            </button>
                ...
                <motion.div key={category.title} variants={fadeUp}>
                  ...
                  <div className="border border-border rounded-lg overflow-hidden">
                    {category.items.map((item, i) => (
                      <div key={item.name + i} className={`flex justify-between items-start px-5 py-4 ${i !== category.items.length - 1 ? "border-b border-border" : ""} hover:bg-secondary/50 transition-colors`}>
